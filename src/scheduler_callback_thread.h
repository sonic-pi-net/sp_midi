...
public:
    ...
    {   
        TimePidIntegerWrapper element(when, pid, integer);
        std::lock_guard<std::mutex> guard(m_MessagesMutex);  
...
private:
    ...
    class TimePidIntegerWrapper {
    public:
        TimePidIntegerWrapper(long long when, ErlNifPid pid, long long integer) : m_when(when), m_pid(pid), m_integer(integer) { };
    ...
    private:
        ...
        friend bool operator<(const TimePidIntegerWrapper& l, const TimePidIntegerWrapper& r) {
            return l.m_when < r.m_when;
        }
    };

    std::set<TimePidIntegerWrapper> m_pending_messages;
